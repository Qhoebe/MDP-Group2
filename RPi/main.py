...
class RaspberryPi:
    ...
    def start(self):
        try:
            ...
            '''
            # buzz STM32 (2 times)
            self.stm_link.send("b")
            time.sleep(3)
            self.stm_link.send("a")
            time.sleep(3)
            self.stm_link.send("a")
            '''
    ...
    def recv_android(self) -> None:
        direction = 1 # 1 is forward, 2 is backward
        while True:
            ...
            # manual movement commands
                ...
                if self.robot_mode.value == 0:  # robot must be in manual mode
                    ...
                    if(message['value'][0:2] == 'FW'):
                        direction = 1
                        self.stm_link.send("f")
                    if(message['value'][0:2] == 'BW'):
                        direction = 2
                        self.stm_link.send("b")
                    if(message['value'][0:2] == 'TL'):
                        if(direction == 1):
                            self.stm_link.send("l")
                        else:
                            self.stm_link.send("x")
                    if(message['value'][0:2] == 'TR'):
                        if(direction == 1):
                            self.stm_link.send("r")
                        else:
                            self.stm_link.send("y")
            ...
            elif message['cat'] == "override":
                if(message['value'][0] == 'F'):
                    dist = (int(message['value'][1]) * 100+ int(message['value'][2])*10 + int(message['value'][3]))
                    self.logger.debug(f"Trying to go for {dist}cm")
                    self.stm_link.send("f")
                    time.sleep(9.5/120*dist)
                    self.stm_link.send("s")
                if(message['value'][0] == 'L'):
                    deg = int(message['value'][1]) * 100+ int(message['value'][2])*10 + int(message['value'][3])
                    self.logger.debug(f"Trying to go for {deg} degree")
                    self.stm_link.send("l")
                    time.sleep(14/360*deg)
                    self.stm_link.send("s")
                if(message['value'][0] == 'R'):
                    deg = (int(message['value'][1]) * 100+ int(message['value'][2])*10 + int(message['value'][3]))
                    #self.logger.debug(f"Trying to go for {deg} degree")
                    self.stm_link.send("r")
                    time.sleep(17.5/360*deg)
                    self.stm_link.send("s")
    ...
    def snap_and_rec(self, obstacle_id: str) -> None:
        ...
        # notify android
        self.logger.info(f"Capturing image for obstacle id: 41")
        self.android_queue.put(AndroidMessage("info", f"Capturing image for obstacle id: 41"))
        ...
        url = f"http://192.168.2.183:4999/imag"
        filename = f"{int(time.time())}_41.jpg"
    ...
    def request_algo(self, data):
        ...
        url = f"http://192.168.2.183:5001/path"
    ...
    def add_navigate_path(self):
        ...
        hardcoded_path: List[str] = [
            "SNAP41", "NOOP" #Snap north
            "Y090", "F030", "R090", "F030", "X090", "SNAP41", "NOOP" 
        ]
        # put commands and paths into queues
        self.clear_queues()
        deg = 0
        dist = 0
        for cmd in hardcoded_path:
            if(cmd[0:4] == 'SNAP'):
                obstacle_id = cmd.replace("SNAP", "")
                self.rpi_action_queue.put(PiAction(cat="snap", value=obstacle_id))
            if(cmd[0] == 'F'):
                dist = (int(cmd[1]) * 100+ int(cmd[2])*10 + int(cmd[3]))
                self.logger.debug(f"Trying to go for {dist}cm")
                self.stm_link.send("f")
                time.sleep(9.5/120*dist)
                self.stm_link.send("s")
            if(cmd[0] == 'L'):
                deg = int(cmd[1]) * 100+ int(cmd[2])*10 + int(cmd[3])
                self.logger.debug(f"Trying to go for {deg} degree")
                self.stm_link.send("l")
                time.sleep(14/360*deg)
                self.stm_link.send("s")
    ...
    def check_api(self) -> bool:
        url = f"http://192.168.2.183:5001/status"
        try:
